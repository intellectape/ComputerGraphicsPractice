...
function initTexture(path, type, number) {
    ...
    if (type) {
        inputTriangles[number].finalTexture = texture;
    } else {
        inputEllipsoids[number].finalTexture = texture;
    }
    ...
    function makeEllipsoid(currEllipsoid, numLongSteps) {
        ...
        try {
            if (numLongSteps % 2 != 0)
                ...
                var ellipsoidUVCoords = [0, -1]; //south pole
                for (var latAngle = -latLimitAngle; latAngle <= latLimitAngle; latAngle += angleIncr) {
                    ...
                    for (var longAngle = 0; longAngle < 2 * Math.PI + angleIncr; longAngle += angleIncr) // for each long
                    {
                        ...
                        ellipsoidUVCoords.push(u, v);
                    }
                } // end for each latitude
                ellipsoidUVCoords.push(0, 1); // north pole
            ...
            return ({ vertices: ellipsoidVertices, normals: ellipsoidNormals, texture: ellipsoidUVCoords, triangles: ellipsoidTriangles });
        } // end try
        catch (e) {
            console.log(e);
        } // end catch
    ...
    try {
        if (inputTriangles == String.null)
            ...
            for (var whichSet = 0; whichSet < numTriangleSets; whichSet++) { // for each tri set
                ...
                inputTriangles[whichSet].type = "triangle"; // model Y axis 
                inputTriangles[whichSet].number = whichSet;
                ...
                if (inputTriangles[whichSet].material.alpha == 1) {
                    inputTriangles[whichSet].opaque = true; // model Y axis 
                    initTexture(inputTriangles[whichSet].material.texture, true, whichSet);
                } else {
                    inputTriangles[whichSet].opaque = false; // model Y axis 
                    initTexture(inputTriangles[whichSet].material.texture, true, whichSet);
                }
            ...
            if (inputEllipsoids == String.null)
                ...
                for (var whichEllipsoid = 0; whichEllipsoid < numEllipsoids; whichEllipsoid++) {
                    ...
                    ellipsoid.number = whichEllipsoid;
                    if (inputEllipsoids[whichEllipsoid].alpha == 1) {
                        inputEllipsoids[whichEllipsoid].opaque = true;
                        initTexture(ellipsoid.texture, false, whichEllipsoid);
                    } else {
                        inputEllipsoids[whichEllipsoid].opaque = false;
                        initTexture(ellipsoid.texture, false, whichEllipsoid);
                    }
...
function sortData() {
    ...
    var numEllipsoids = inputEllipsoids.length; // remember how many ellipsoids
    for (var whichSet = 0; whichSet < numTriangleSets; whichSet++) { // for each tri set
        if (inputTriangles[whichSet].opaque) {
            opaqueMods.push(inputTriangles[whichSet]);
        } else {
            transparentMods.push(inputTriangles[whichSet]);
        }
    }
    for (var whichEllipsoid = 0; whichEllipsoid < numEllipsoids; whichEllipsoid++) {
        if (inputEllipsoids[whichEllipsoid].opaque) {
            opaqueMods.push(inputEllipsoids[whichEllipsoid]);
        } else {
            transparentMods.push(inputEllipsoids[whichEllipsoid]);
        }
    }

    for (var whichModel = 0; whichModel < opaqueMods.length; whichModel++) {
        totalModels.push(opaqueMods[whichModel]);
    }

    transparentMods.sort(function(a, b) {
        var diffA = a.center[2] + a.translation[2];
        var diffB = b.center[2] + b.translation[2];
        return diffA - diffB;
    });

    for (var whichModel = 0; whichModel < transparentMods.length; whichModel++) {
        totalModels.push(transparentMods[whichModel]);
    }

    for (var whichModel = 0; whichModel < totalModels.length; whichModel++) {
        document.getElementById("l" + whichModel).innerText = totalModels[whichModel].type + (totalModels[whichModel].number + 1);
    }
...
function setupShaders() {
    ...
    var fShaderCode = `
        ...
        void main(void) {
            ...
            vec3 lightOut = uLightAmbient + uLightDiffuse*lambert;
            if (textureFlag == 0) {
                ...
                vec4 textureColor = texture2D(uSampler, vec2(vTextureCoord.s, vTextureCoord.t)); 
                textureColor.a = textureColor.a *  uAlpha;
                gl_FragColor = vec4(colorOut * textureColor.rgb, 1.0 * textureColor.a);
            }
            else {
                vec4 textureColor = texture2D(uSampler, vec2(vTextureCoord.s, vTextureCoord.t)); 
                textureColor.a = textureColor.a *  uAlpha;
                vec3 tempColor = uLightAmbient + uLightDiffuse*lambert + uLightSpecular*highlight;
                gl_FragColor = vec4(tempColor * textureColor.rgb, 1.0 * textureColor.a);
            }       
        }
    `;
    ...
    try {
        ...
        if (!gl.getShaderParameter(fShader, gl.COMPILE_STATUS)) { // bad frag shader compile
            ...
            if (!gl.getProgramParameter(shaderProgram, gl.LINK_STATUS)) { // bad program link
                ...
                textureFlag = gl.getUniformLocation(shaderProgram, "textureFlag");
                uniformAlpha = gl.getUniformLocation(shaderProgram, "uAlpha");
...
function renderModels() {
    ...
    var numModels = totalModels.length;
    for (var whichModel = 0; whichModel < numModels; whichModel++) {
        if (totalModels[whichModel].type == "triangle") {
            currSet = totalModels[whichModel];
            ...
            gl.bindBuffer(gl.ARRAY_BUFFER, vertexBuffers[currSet.number]); // activate
            gl.vertexAttribPointer(vPosAttribLoc, 3, gl.FLOAT, false, 0, 0); // feed
            gl.bindBuffer(gl.ARRAY_BUFFER, normalBuffers[currSet.number]); // activate
            gl.vertexAttribPointer(vNormAttribLoc, 3, gl.FLOAT, false, 0, 0); // feed

            gl.bindBuffer(gl.ARRAY_BUFFER, vertexTextureCoordBuffer[currSet.number]);
            gl.vertexAttribPointer(textureCoordAttribute, 2, gl.FLOAT, false, 0, 0);

            if (currSet.finalTexture.isLoaded) {
                gl.activeTexture(gl.TEXTURE0);
                gl.bindTexture(gl.TEXTURE_2D, currSet.finalTexture);
                gl.uniform1i(samplerUniform, 0);
            }

            gl.uniform1i(textureFlag, toggleTexture);
            ...
            gl.uniform1f(uniformAlpha, parseFloat(currSet.material.alpha));


            // triangle buffer: activate and render
            gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, triangleBuffers[currSet.number]); // activate
            gl.drawElements(gl.TRIANGLES, 3 * triSetSizes[currSet.number], gl.UNSIGNED_SHORT, 0); // render

        } else {
            ellipsoid = totalModels[whichModel];
            ...
            gl.bindBuffer(gl.ARRAY_BUFFER, vertexBuffers[numTriangleSets + ellipsoid.number]); // activate vertex buffer
            gl.vertexAttribPointer(vPosAttribLoc, 3, gl.FLOAT, false, 0, 0); // feed vertex buffer to shader
            gl.bindBuffer(gl.ARRAY_BUFFER, normalBuffers[numTriangleSets + ellipsoid.number]); // activate normal buffer
            ...
            gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, triangleBuffers[numTriangleSets + ellipsoid.number]); // activate tri buffer

            if (ellipsoid.finalTexture.isLoaded) {
                gl.activeTexture(gl.TEXTURE0);
                gl.bindTexture(gl.TEXTURE_2D, ellipsoid.finalTexture);
                gl.uniform1i(samplerUniform, 0);
            }
            gl.uniform1i(textureFlag, toggleTexture);
            ...
            gl.uniform1f(uniformAlpha, parseFloat(totalModels[whichModel].alpha));
            // draw a transformed instance of the ellipsoid
            gl.drawElements(gl.TRIANGLES, triSetSizes[numTriangleSets + ellipsoid.number], gl.UNSIGNED_SHORT, 0); // render
        }
    }
} // end render model
